...
void CMonoPlugin::two_version_comparison(int framenum, IResultsServer* results_server) {
    ...
    /*cout << "doing ME with prev\n";
    cMV* MtoPREV = m_analyser.GetME(5)->Field(0);
    cout << "done ME with prev\n";
    ...
    cout << "extracted channels\n";
      */
    BYTE* estimated_depth_map = (BYTE*)malloc(m_height*m_width * sizeof(BYTE));
    //BYTE* depth_quality_map = (BYTE*)malloc(m_height*m_width * sizeof(BYTE));
    ...
    m_analyser.M_TO_PREV.GetDepthMap(estimated_depth_map, 0);
    //depth_quality_map = m_analyser.M_TO_PREV.Quality(0);


    cout << "estimated depth\n";
    //log.vis_normalized(estimated_depth_map, m_height, m_width, "DEPTH.png", framenum);
    //log.vis_normalized(depth_quality_map, m_height, m_width, "QUALITY.png", framenum);
   ...
   /* free(MtoPREV);
    ...
    free(Left_U);
    free(Left_V);*/
    free(estimated_depth_map);
    //free(depth_quality_map);
