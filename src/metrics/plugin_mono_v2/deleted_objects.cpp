...
bool CMonoPlugin::color_check(PNG_Image *a, PNG_Image *b, const int i, const int j) {
	BYTE dB = abs(a->b(j, i) - b->b(j, i));
	BYTE dG = abs(a->g(j, i) - b->g(j, i));
	BYTE dR = abs(a->r(j, i) - b->r(j, i));
...
double CMonoPlugin::FindDeletedTwoVersions(BYTE* depth_map, int framenumber) {
	int sum = 0;
