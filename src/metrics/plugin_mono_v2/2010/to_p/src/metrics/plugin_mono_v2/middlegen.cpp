...
void MiddleGen::AddValue(double x, int y, BYTE p, BYTE m)
{
	int l_x = (int)floor(x);
	int r_x = (int)floor(x+0.99);
	if(l_x < 0 || y < 0 || r_x > m_width - 1 || y > m_height - 1)
		return;
	double l_dist = CLIP(1-(x - l_x), 0, 1);
	double r_dist = CLIP(1-(r_x - x), 0, 1);
...
void MiddleGen::MakeView(me_data left, me_data right, double pos)
{
	...
	for(int j = 0; j < m_height; j++)
		for(int i = 0; i < m_width; i++)
		{
			...
			double new_xl = i + l_vec[index].x*pos/4.0f;
			AddValue(new_xl, j, left.frame->y(i,j), l_mask[index]);
			double new_xr = i + r_vec[index].x*(1.0-pos)/4.0f;
	...
	for(int j = 0; j < m_height; j++)
		for(int i = 0; i < m_width; i++)
		{
			...
			{
				m_image[index] = TO_BYTE(cur.ys / cur.ds);
				m_mask[index] = TO_BYTE(cur.ms / cur.ds);
			}
		}
}
