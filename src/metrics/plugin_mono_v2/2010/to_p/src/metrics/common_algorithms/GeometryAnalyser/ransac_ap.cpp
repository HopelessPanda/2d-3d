...
aTransform Ransac::findTransform(vecPairArray src)
{
	...
	cv::Mat matrix_inv(3, 3, CV_64F);
	cv::Mat column(3, 1, CV_64F);
	cv::Mat result(3, 1, CV_64F);

	for (int i = 0; i < 3; i++)
	{
		double z = m_type == GEOMETRY ? src.array[i].sY + src.array[i].vY / 4.0f : src.array[i].sX + src.array[i].vX / 4.0f;
		matrix.at<double>(i, 0) = src.array[i].sX;
		if (m_type == GEOMETRY)
			matrix.at<double>(i, 1) = src.array[i].sY;
		else
			matrix.at<double>(i, 1) = src.array[i].mX;
		matrix.at<double>(i, 2) = 1.0f;
		column.at<double>(i) = z;
		result.at<double>(i) = 0;
	}
